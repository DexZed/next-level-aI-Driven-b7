...
async function createUsers() {
    ...
    users = (await db.orm.public.User.select("id", "email", "name", "password_hash", "role").createAll(userData));
}
async function createCategoriesAndServices() {
    ...
    categories = (await db.orm.public.Category.select("id", "name", "description", "is_active").createAll(categoryData));
    ...
    services = (await db.orm.public.Service.select("id", "name", "description", "category_id").createAll(serviceData));
}
async function createTechnicianProfiles() {
    ...
    technicians = (await db.orm.public.Technician.select("id", "user_id", "bio", "is_available", "ratingAvg").createAll(technicianData));
}
